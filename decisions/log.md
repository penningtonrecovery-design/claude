...
## 2026-08-15 — Built the social workflow in n8n (workflow 7EfDHYnUez8NDsEx)
**Decision:** Implemented the "one idea → all platforms" workflow directly in the n8n instance rather than only handing over a builder prompt. Claude (Anthropic Chat Model + structured output parser) writes the four posts; raw form submissions are logged to an n8n Data Table ("Social Post Log", id 6Axa1dqkEGphZApf); Gmail send-and-wait provides the approval gate with a 7-day window; publishing nodes use continue-on-error so one platform's failure never blocks the rest.
**Reasoning:** The n8n MCP connection made direct construction possible and testable. A Data Table beats Google Sheets here because it needs zero extra setup. Claude was chosen over OpenAI because Anthropic credentials already exist in the instance.
**Context:** Simulated end-to-end test passed. Two blockers remain for the user: both Anthropic API keys are out of credits, and Facebook/Instagram, LinkedIn, and X credentials plus their account IDs still need to be connected in the n8n editor.
